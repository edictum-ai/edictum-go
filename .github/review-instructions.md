...
| # | File | Issue | Violates |
|---|------|-------|----------|
| 1 | `session/session.go:15` | Description | [CLAUDE.md — Terminology](CLAUDE.md) |

<details>
<summary>Details</summary>

**1. `session/session.go:15` — Short title**
...
| # | File | Suggestion |
|---|------|------------|
| 1 | `pipeline/pipeline.go` | Description |

<details>
<summary>Details</summary>

**1. `pipeline/pipeline.go` — Short title**
...
```markdown
- ✏️ `pipeline/pipeline.go` (modified)
- ✨ `.github/workflows/review.yml` (new)
- 🗑️ `internal/shlex/old.go` (deleted)
```

### Checks applied

`{checks_applied}`: Comma-separated list of check categories that were relevant, e.g.:
`Core boundary · Code quality · Terminology · Security · Concurrency · Adapter parity`
